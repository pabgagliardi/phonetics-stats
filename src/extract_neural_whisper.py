"""
Stage 5 — extract_neural_whisper.py (optimised with checkpointing)
Caches Whisper encoder outputs per WAV file so each file
is only processed ONCE, then slices frames per phoneme.

Optimisations:
- processes one WAV file at a time (encoder runs once per file)
- truncates audio to 30s max (Whisper's context window)
...
OUTPUT_DIR      = "data"
CHECKPOINT_PATH = "data/whisper_checkpoint.npz"
WHISPER_SR      = 16000
WHISPER_HOP_SEC = 0.02  # 20ms hop
...
def load_wav(wav_path):
    ...
    if sample_rate != WHISPER_SR:
        target_len = int(len(audio) * WHISPER_SR / sample_rate)
        audio = np.interp(
            np.linspace(0, len(audio), target_len),
            np.arange(len(audio)),
            audio
        )
    # truncate to 30s max (Whisper's context window)
    max_samples = WHISPER_SR * 30
...
def get_overlapping_steps(onset, offset, n_frames):
    frame_times = np.arange(n_frames) * WHISPER_HOP_SEC
...
for layer in LAYERS:
    ...
    out_path = f"{OUTPUT_DIR}/features_whisper_layer{layer}.npz"
